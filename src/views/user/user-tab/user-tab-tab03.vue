<template>
  <div>
    <table class="table table-box table-sm">
      ...
      <tr>
        <td class="titleColumn">
          ...
          {{imgText.general.sshi0021.businessSite}}
        </td>
        <td colspan="5">
          <b-row>
            <b-col md="10">
      ...
      <tr>
        ...
        <td colspan="5">
          <b-input-group size="sm">
            <b-form-select v-model="user.roleCode" :options="dataCombobox" @change="onChange(true)"></b-form-select>
          </b-input-group>
        </td>
        <td class="titleColumn">
          <span class="asterisk"></span>
      ...
      <tr>
        ...
        <td colspan="12" class="form-checkbox">
          <b-form-checkbox
            class="checkbox"
            v-model="user.notifyReport"
            @input="checkRequiredEmail"
            @change="onChange(true)"
          >終了報告</b-form-checkbox>
          <b-form-checkbox
            class="checkbox"
            v-model="user.notifyApprove"
            @input="checkRequiredEmail"
            @change="onChange(true)"
          >終了報告承認依頼</b-form-checkbox>
          <b-form-checkbox
            class="checkbox"
            v-model="user.notifyExpire"
            @input="checkRequiredEmail"
            @change="onChange(true)"
          >報告期限切れ</b-form-checkbox>
          <b-form-checkbox
            class="checkbox"
            v-model="user.notifyExpireSoon"
            @input="checkRequiredEmail"
            @change="onChange(true)"
          >報告期限切れ予告</b-form-checkbox>
          <b-form-checkbox
            class="checkbox"
            v-model="user.notifyLicenseExpire"
            @input="checkRequiredEmail"
            @change="onChange(true)"
          >許可証期限切れ</b-form-checkbox>
          <b-form-checkbox
            class="checkbox"
            v-model="user.notifyLicenseExpireSoon"
            @input="checkRequiredEmail"
            @change="onChange(true)"
          >許可証期限切れ予告</b-form-checkbox>
          <b-form-checkbox
            class="checkbox"
            v-model="user.notifyDone"
            @input="checkRequiredEmail"
            @change="onChange(true)"
          >完了通知</b-form-checkbox>
        </td>
      </tr>
      <tr>
    ...
    <BaseButtonGroup
      ...
      @onBack="onBack"
    />
    <UserPopup01 @onSendDataBranch="onSendDataBranch" :popupId="popupId" />
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      popupId: "sshi0021-popup",
      emitSaveOrUpdate: "onSaveOrUpdateTabThree",
      isChangeData: false
    };
  },
  created() {
  ...
  methods: {
    onBack() {
      this.$emit("onBack", "userTab3");
    },
    onAddUpdate() {
      if (!this.status) {
        this.userTmp = { ...this.user };
        this.userTmp.usertypeCode = 3;
        this.userTmp.customerCode = this.customerCode;
        this.userTmp.userCode = 1;
        this.$emit(this.emitSaveOrUpdate, this.userTmp);
        return;
      }
      this.$emit(this.emitSaveOrUpdate, this.user);
    },
    ShowPopup() {
