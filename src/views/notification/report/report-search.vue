          ...
          <b-col>
            <b-form-group>
              <b-form-checkbox-group
                id="checkbox-group-report"
                name="checkbox-notify-type"
                v-model="selectedNotifyType"
              >
                <b-form-checkbox
                  :value="notifyType.tranferEnd.key"
                >{{ imgtext.general.fshi0012.tranferEndCheckbox }}</b-form-checkbox>
                <b-badge
                  pill
                  variant="success badge-card"
                  :class="classByWaste"
                  v-if="totalTranferEnd > 0"
                >{{ totalTranferEnd }}</b-badge>
                <b-form-checkbox
                  class="ml-3"
                  :value="notifyType.treatmentEnd.key"
                >{{ imgtext.general.fshi0012.treatmentEndCheckbox }}</b-form-checkbox>
                <b-badge
                  pill
                  variant="success badge-card"
                  :class="classByWaste"
                  v-if="totalTreatmentEnd > 0"
                >{{ totalTreatmentEnd }}</b-badge>
                <b-form-checkbox
                  class="ml-3"
                  :value="notifyType.treatmentFinal.key"
                >{{ imgtext.general.fshi0012.treatmentFinalCheckbox }}</b-form-checkbox>
                <b-badge
                  pill
                  variant="success badge-card"
                  :class="classByWaste"
                  v-if="totalTreatmentFinal > 0"
                >{{ totalTreatmentFinal }}</b-badge>
              </b-form-checkbox-group>
            </b-form-group>
          </b-col>
          <b-col></b-col>
...
export default {
  props: ['condition', 'wasteClass', 'options', 'totalTranferEnd', 'totalTreatmentEnd', 'totalTreatmentFinal', 'classByWaste'],
