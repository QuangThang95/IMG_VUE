...
import RequestApprovalSearch from './request-approval-search.vue'
import RequestApprovalList from './request-approval-list.vue'
...
export default {
  ...
  data() {
    return {
      ...
      message: [],
      title: IMText.title.sshi0012Request,
      openDl: false,
      currentCustomer: current.customer(),
      dataWasteClass: WasteClass,
      notifyClass: NotifyClass.request.key,
      resultCount: 0,
  ...
  async created() {
    ...
    await this.getTotalTranferForNotification(NotifyType.tranfer.key, this.condition);
    await this.getTotalEditForNotification(NotifyType.edit.key, this.condition);
    this.onReset();
  },
  ...
  methods: {
    ...
    async onSearch(conditionSearch) {
      ...
      if (this.validateSearch(conditionSearch)) {
        ...
        await this.getTotalTranferForNotification(NotifyType.tranfer.key, conditionSearch);
        await this.getTotalEditForNotification(NotifyType.edit.key, conditionSearch);
    ...
    async getTotalTranferForNotification(notifyType, condition) {
      // this.openDl = true;
      let params = {
      ...
      await this.$store.dispatch('notificationModule/getTotalTranferForNotification', { notifyType, params }).finally(() => {
        // W.i.p @DKHuyen
        // this.openDl = false;
      });
    },

    async getTotalEditForNotification(notifyType, condition) {
      // W.i.p @DKHuyen
      // this.openDl = true;
      let params = {
        currentBranchCode: condition.branchCode !== 0 ? condition.branchCode : null,
      ...
      await this.$store.dispatch('notificationModule/getTotalEditForNotification', { notifyType, params }).finally(() => {
        // W.i.p @DKHuyen
        // this.openDl = false;
      });
    },
  ...
  computed: {
    ...
    totalTranfer() {
      return this.$store.getters['notificationModule/getTotalTranfer'];
    },

    totalEdit() {
      return this.$store.getters['notificationModule/getTotalEdit'];
    },
