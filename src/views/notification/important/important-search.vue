          ...
          <b-col>
            <b-form-group>
              <b-form-checkbox-group
                id="checkbox-group-important"
                name="checkbox-notify-type"
                v-model="selectedNotifyType"
              >
                <b-form-checkbox
                  :value="notifyType.reportExpired.key"
                >{{ imgtext.general.fshi0012.reportExpiredCheckbox }}</b-form-checkbox>
                <b-badge
                  pill
                  variant="danger badge-card"
                  :class="classByWaste"
                  v-if="totalReportExpired > 0"
                >{{ totalReportExpired }}</b-badge>
                <b-form-checkbox
                  class="ml-3"
                  :value="notifyType.notifyExpired.key"
                >{{ imgtext.general.fshi0012.notifyExpiredCheckbox }}</b-form-checkbox>
                <b-badge
                  pill
                  variant="warning badge-card"
                  :class="classByWaste"
                  v-if="totalNotifyExpired > 0"
                >{{ totalNotifyExpired }}</b-badge>
                <b-form-checkbox
                  class="ml-3"
                  :value="notifyType.correctCancel.key"
                >{{ imgtext.general.fshi0012.correctCancelCheckbox }}</b-form-checkbox>
                <b-badge
                  pill
                  variant="secondary badge-card"
                  :class="classByWaste"
                  v-if="totalCorrectCancel > 0"
                >{{ totalCorrectCancel }}</b-badge>
              </b-form-checkbox-group>
            </b-form-group>
          </b-col>
          <b-col></b-col>
...
export default {
  props: ['condition', 'wasteClass', 'options', 'totalReportExpired', 'totalNotifyExpired', 'totalCorrectCancel', 'classByWaste'],
