  ...
  <div>
    <b-container fluid>
      ...
      <ImportantSearch
        ...
        :options="options"
        :totalReportExpired="totalReportExpired"
        :totalNotifyExpired="totalNotifyExpired"
        :totalCorrectCancel="totalCorrectCancel"
...
<script>
import ImportantSearch from "./important-search.vue";
import ImportantList from "./important-list.vue";
...
export default {
  ...
  data() {
    return {
      ...
      message: [],
      title: IMText.title.sshi0012Important,
      openDl: false,
      currentCustomer: current.customer(),
      dataWasteClass: WasteClass,
      notifyClass: NotifyClass.important.key,
      resultCount: 0,
  ...
  async created() {
    ...
    await this.getListBranchForNotification(
      this.currentCustomer.customerCode,
      this.condition.wasteClass
    );
    await this.getTotalReportExpiredForNotification(
      NotifyType.reportExpired.key,
      this.condition
    );
    await this.getTotalNotifyExpiredForNotification(
      NotifyType.notifyExpired.key,
      this.condition
    );
    await this.getTotalCorrectCancelForNotification(
      NotifyType.correctCancel.key,
      this.condition
    );
    this.onReset();
  },
  ...
  methods: {
    ...
    async searchNotifications(condition) {
      this.openDl = true;
      await this.$store
        .dispatch("notificationModule/searchNotifications", {
          ...
          dateFrom: condition.dateFrom
            ? moment(String(condition.dateFrom)).format(
                DateTimeFormat.DATE_MMDDYYYY
              )
            : null,
          dateTo: condition.dateTo
            ? moment(String(condition.dateTo)).format(
                DateTimeFormat.DATE_MMDDYYYY
              )
            : null,
    ...
    async getListBranchForNotification(customerCode, wasteClass) {
      ...
      // this.openDl = true;
      await this.$store
        .dispatch("notificationModule/getListBranchForNotification", {
          customerCode,
          wasteClass
        })
        .finally(() => {
    ...
    async getTotalReportExpiredForNotification(notifyType, condition) {
      // W.i.p @DKHuyen
      // this.openDl = true;
      let params = {
        currentBranchCode:
          condition.branchCode !== 0 ? condition.branchCode : null,
        currentWasteClass: condition.wasteClass,
        ...
        dateFrom: condition.dateFrom
          ...
          : null,
        dateTo: condition.dateTo
          ? moment(String(condition.dateTo)).format(DateTimeFormat.DATE_PARAMS)
          : null,
        currentCustomerCode: condition.customerCode
      };
      await this.$store
        .dispatch("notificationModule/getTotalReportExpiredForNotification", {
          notifyType,
          params
        })
        .finally(() => {
    ...
    async getTotalNotifyExpiredForNotification(notifyType, condition) {
      // W.i.p @DKHuyen
      // this.openDl = true;
      let params = {
        currentBranchCode:
      ...
      await this.$store
        .dispatch("notificationModule/getTotalNotifyExpiredForNotification", {
          notifyType,
          params
        })
        .finally(() => {
    ...
    async getTotalCorrectCancelForNotification(notifyType, condition) {
      // W.i.p @DKHuyen
      // this.openDl = true;
      let params = {
        currentBranchCode:
      ...
      await this.$store
        .dispatch("notificationModule/getTotalCorrectCancelForNotification", {
          notifyType,
          params
        })
        .finally(() => {
    ...
    async getDischargeWasteClassByBranch(customerCode, branchCode) {
      this.openDl = true;
      await this.$store
        .dispatch("notificationModule/getDischargeWasteClassByBranch", {
          customerCode,
          branchCode
        })
        .finally(() => {
    ...
    getWasteClassUrl() {
      ...
      this.condition.wasteClass =
        wasteClassUrl !== WasteClass.all.key ? wasteClassUrl : null;
    },
      ...
      for (const prop in dataWasteClass) {
        if (dataWasteClass[prop].id !== 0 && dataWasteClass[prop].id <= 3) {
          listButton.push({
            ...
            state:
              this.condition.wasteClass === dataWasteClass[prop].key
                ? true
                : false
          });
        }
  ...
  computed: {
    ...
    totalReportExpired() {
      return this.$store.getters["notificationModule/getTotalReportExpired"];
    },

    totalNotifyExpired() {
      return this.$store.getters["notificationModule/getTotalNotifyExpired"];
    },

    totalCorrectCancel() {
      return this.$store.getters["notificationModule/getTotalCorrectCancel"];
    },
